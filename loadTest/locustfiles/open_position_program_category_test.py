...
def validate_response(data):
    ...
    assert data["count"] == 38, f"Expected count 38, but got {data['count']}."
    assert len(data["results"]) == 38, f"Expected 38 results, but got {len(data['results'])}."
...
class OpenPositionProgramCategoryTestUser:
    def __init__(self, client):
        self.client = client
        self.BASE_URL = "https://kariyer.baykartech.com"

    def get_program_category_list(self):
        endpoint = "/tr/application/serializerProgramCategory/"
...
class CombinedUser(HttpUser):
    ...
    @task
    def get_program_category(self):
        test_user = OpenPositionProgramCategoryTestUser(self.client)
        test_user.get_program_category_list()
